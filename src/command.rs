...
impl<'d> CmdRecord<'d>
{
	...
	pub fn fill_buffer(&mut self, dst: &Buffer, dst_offset: usize, size: usize, data: u32) -> &mut Self
	{
		unsafe
		{
			Resolver::get().cmd_fill_buffer(self.ptr.native_ptr(), dst.native_ptr(), dst_offset as _, size as _, data);
		}
		self
	}
...
impl<'d> CmdRecord<'d>
{
	...
	pub fn pipeline_barrier(&mut self,
		src_stage_mask: PipelineStageFlags, dst_stage_mask: PipelineStageFlags, by_region: bool,
...
impl<'d> CmdRecord<'d>
{
	...
	pub fn copy_query_pool_results(&mut self,
		pool: &QueryPool, range: Range<u32>, dst: &Buffer, dst_offset: usize,
...
impl ImageMemoryBarrier
{
	...
	pub fn new(img: &ImageSubref, old_layout: ImageLayout, new_layout: ImageLayout) -> Self
	{
		ImageMemoryBarrier(VkImageMemoryBarrier
		{
			...
			dstAccessMask: new_layout.default_access_mask(),
			.. Default::default()
		})
	}
	...
	pub fn new_raw<SR>(res: &Image, subres: &SR, old: ImageLayout, new: ImageLayout) -> Self
		where SR: AsRef<VkImageSubresourceRange>
	{
		ImageMemoryBarrier(VkImageMemoryBarrier
		{
			image: res.native_ptr(), subresourceRange: subres.as_ref().clone(),
			oldLayout: old as _, newLayout: new as _,
			srcAccessMask: old.default_access_mask(), dstAccessMask: new.default_access_mask(),
			.. Default::default()
		})
	}
...
{
	fn into(self) -> VkImageMemoryBarrier { self.0 }
}
impl From<VkImageMemoryBarrier> for ImageMemoryBarrier
{
	fn from(v: VkImageMemoryBarrier) -> Self { ImageMemoryBarrier(v) }
}
...
{
	fn into(self) -> VkBufferMemoryBarrier { self.0 }
}
impl From<VkBufferMemoryBarrier> for BufferMemoryBarrier
{
	fn from(v: VkBufferMemoryBarrier) -> Self { BufferMemoryBarrier(v) }
}
