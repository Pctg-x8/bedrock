...
#[cfg(feature = "Implements")] use crate::{
	VkResultHandler, fnconv::FnTransmute,
	DescriptorSetWriteInfo, DescriptorSetCopyInfo, Fence,
...
impl<'p> DeviceBuilder<'p>
{
	pub fn new(pdev: &'p PhysicalDevice) -> Self
	{
		DeviceBuilder
		{
			pdev_ref: pdev, queue_infos: Vec::new(), layers: Vec::new(), extensions: Vec::new(),
	...
	{
		self.extensions.push(std::ffi::CString::new(name).unwrap()); self
	}
	...
	{
		let qinfos = self.queue_infos.iter()
		...
		unsafe
		{
			Resolver::get()
				...
				.into_result()
				.map(|_| Device(RefCounter::new(DeviceCell(h.assume_init(), self.pdev_ref.parent().clone()))))
		}
	}
}
/// Tweaking features
...
impl Device
{
	...
	pub fn update_descriptor_sets(&self, write: &[DescriptorSetWriteInfo], copy: &[DescriptorSetCopyInfo])
	{
		...
		let w = wt.iter().map(|&(set, binding, array, dty, count, ref iv, ref bv, ref bvv)| VkWriteDescriptorSet
		{
			...
			pImageInfo: iv.as_ptr(), pBufferInfo: bv.as_ptr(), pTexelBufferView: bvv.as_ptr(),
			.. Default::default()
		}).collect::<Vec<_>>();
		let c = copy.iter().map(|x| VkCopyDescriptorSet
		{
			srcSet: x.src.0, srcBinding: x.src.1, srcArrayElement: x.src.2,
			dstSet: x.dst.0, dstBinding: x.dst.1, dstArrayElement: x.dst.2, descriptorCount: x.count,
			.. Default::default()
		}).collect::<Vec<_>>();
		unsafe
		{
			Resolver::get()
				.update_descriptor_sets(self.native_ptr(), w.len() as _, w.as_ptr(), c.len() as _, c.as_ptr());
		}
	}
}
...
impl<'s> Default for SparseBindingOpBatch<'s>
{
	fn default() -> Self
	{
		SparseBindingOpBatch
		{
			...
			buffer_binds: Cow::Borrowed(&[]), image_opaque_binds: Cow::Borrowed(&[]), image_binds: Cow::Borrowed(&[]),
...
impl Queue
{
	...
	{
		let sem_ptrs = batches.iter()
			.map(|x| (x.wait_semaphores.iter().map(|&x| x.native_ptr()).collect(), x.signal_semaphores.iter().map(|&x| x.native_ptr()).collect()));
		let batches: Vec<_> = batches.iter().zip(sem_ptrs).map(|(x, (ws, ss)): (&SparseBindingOpBatch, (Vec<_>, Vec<_>))| VkBindSparseInfo
		{
		...
		unsafe
		{
			Resolver::get()
				.queue_bind_sparse(
					self.0, batches.len() as _, batches.as_ptr(),
...
impl Queue
{
	...
	pub fn submit(&self, batches: &[SubmissionBatch], fence: Option<&Fence>) -> crate::Result<()>
	{
		...
		unsafe
		{
			Resolver::get()
				.queue_submit(
					self.native_ptr(), batches.len() as _, batches.as_ptr(),
					fence.map(VkHandle::native_ptr).unwrap_or_else(std::ptr::null_mut)
				)
				.into_result()
		}
